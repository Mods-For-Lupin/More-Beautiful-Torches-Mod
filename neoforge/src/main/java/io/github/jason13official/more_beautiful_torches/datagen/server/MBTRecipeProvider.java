...
public class MBTRecipeProvider extends RecipeProvider {
  ...
  protected void buildRecipes(RecipeOutput recipeOutput) {
    for (ModBlocks.TorchEntry entry : ModBlocks.TORCHES) {
      torch(recipeOutput, entry.standing(), entry.source(), Ingredient.of(Items.STICK));
    }
    for (ModBlocks.TorchEntry entry : ModBlocks.SOUL_TORCHES) {
      torch(recipeOutput, entry.standing(), entry.source(), Ingredient.of(Blocks.SOUL_SOIL, Blocks.SOUL_SAND));
    }
    for (ModBlocks.TorchEntry entry : ModBlocks.REDSTONE_TORCHES) {
      torch(recipeOutput, entry.standing(), entry.source(), Ingredient.of(Items.REDSTONE));
    }
  }

  private void torch(RecipeOutput recipeOutput, Block standing, Block source, Ingredient catalyst) {
